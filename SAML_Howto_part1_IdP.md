...
First, let's introduce some specific vocabulary:
    ...
    * SAML is a markup language used by Service Providers and Identity Providers to   communicate among each other, namely to exchange assertions (or attributes) about a user  from the IdP to the SP.
...
Identity Providers and Service Providers usually interact according to the following scenario:
    ...
    2. The resource, or more precisely Service Provider, gives Alice a list of    Identity providers it is aware of. Acme Corp's IdP is among the list, so    Alice chooses this one. 
...
This describes the current implementation in Icehouse. This is subject to change in Juno and beyond !
...
It is possible to skip this part: you can set up a Service Provider and use it with the test IdP at ``http://www.testshib.org`` if you only want to test the federation workflow on keystone. It greatly simplifies things as it helps you configure your SP, but it won't work with a bogus FQDN though.  
...
Create certificates with the right FQDN by following the instructions here: https://wiki.shibboleth.net/confluence/display/SHIB2/IdPCertRenew
...
The following resources can be useful for troubleshooting and more information about shibboleth IdPs:
    ...
    * https://www.acceleratecarboncapture.org/drupal/blog/patton/public/shibboleth-test-bed-part-2-installing-idp
    * https://wiki.shibboleth.net/confluence/display/SHIB/LogFiles
